...
# Initialize session state to store saved answers if not already present
...
if uploaded_file:
    ...
    # Only show question input if PDF text was successfully extracted
    if pdf_text:
        # Text input for the user's question
        question = st.text_input("Ask a question about the PDF content")

        # Button to get an answer from the AI
        if st.button("Get Answer") and question:
            # Show a loading spinner while waiting for the AI response
            with st.spinner('Getting answer from AI...'):
                answer = ask_question_to_gemini(pdf_text, question) # Call the Gemini-powered function
                st.write("**Answer:**", answer)

                # Show "Save Answer" button only if a valid answer was retrieved
                # Check for the specific error message to avoid saving it
                if answer and "Could not get an answer" not in answer:
...
# Display saved answers if any exist
if st.session_state.saved_answers:
    ...
    # Layout buttons in two columns for better spacing
    ...
    with col2:
        if st.button("📄 Export All as PDF"):
            # Export saved answers to a PDF and offer it for download
            ...
            os.remove(pdf_path) # Clean up the temporary PDF file after download
